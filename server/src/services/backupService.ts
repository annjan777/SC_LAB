...
export function createBackupArchive(): Archiver {
  ...
  archive.append(
    JSON.stringify({ created_at: new Date().toISOString(), app: 'sc-lab-portal' }, null, 2),
    { name: 'manifest.json' }
  );
...
function resolveSafeExtractPath(baseDir: string, entryName: string): string {
 ...
 */
export async function restoreFromBackup(zipPath: string): Promise<{ preImportBackupPath: string }> {
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        const restoreProcess = spawn(
          'pg_restore',
          ['--clean', '--if-exists', '--no-owner', '--single-transaction', ...dbConnArgs(), dumpFilePath],
          { env: dbEnv() }
        );
        let stderr = '';
        ...
        restoreProcess.on('close', (code) => {
          if (code !== 0) reject(new Error(`pg_restore exited with code ${code}: ${stderr}`));
          else resolve();
        });
      });
    } finally {
