...
router.post('/', authenticate, upload.single('image'), async (req: Request, res: Response) => {
  try {
    ...
    if (rawKeys.length === 0) return res.status(400).json({ error: 'No fields to insert' });
    ...
    const result = await query(
      `INSERT INTO facilities (${safeKeys.map(k => `"${k}"`).join(',')}) VALUES (${placeholders.join(',')}) RETURNING *`,
      values
    );
...
router.put('/:id', authenticate, upload.single('image'), async (req: Request, res: Response) => {
  try {
    ...
    const result = await query(
      `UPDATE facilities SET ${setClause} WHERE id = $${values.length} RETURNING *`,
      values
    );
    res.json(result.rows[0]);
  } catch (err: any) {
