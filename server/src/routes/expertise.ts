...
type ExpertiseType = keyof typeof EXPERTISE_TABLES;

function expertiseRoutes(type: ExpertiseType) {
  const { table, nameCol, levelCol } = EXPERTISE_TABLES[type];
  const r = Router();

  // GET /api/expertise/:type?user_id=...
  r.get('/', authenticate, async (req: Request, res: Response) => {
    try {
  ...
  r.post('/', authenticate, async (req: Request, res: Response) => {
    try {
      ...
      const result = await query(
        `INSERT INTO ${table} (user_id, "${nameCol}", "${levelCol}") VALUES ($1, $2, $3) RETURNING *`,
        [userId, name, level]
      );
  ...
  r.delete('/:id', authenticate, async (req: Request, res: Response) => {
    try {
...
router.get('/suggestions', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const term = req.query.search_term as string || req.query.term as string || '';
    ...
    res.json(result.rows.map((r: any) => r.suggestion));
  } catch (err: any) {
    console.error(err); res.status(500).json({ error: 'Internal Server Error' });
  }
});
...
router.get('/autocomplete/:type', authenticate, async (req: Request, res: Response) => {
  try {
