...
async function recalculateProgressFromMilestones(client: { query: (text: string, params?: any[]) => Promise<any> }, workId: string) {
  const milestonesResult = await client.query(
    'SELECT status FROM work_milestones WHERE work_id = $1',
    [workId]
  );
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    delete fields.id; delete fields.created_at; delete fields.user_id;
    ...
    if (!hasManagerPerm && req.user!.user_role !== 'admin' && req.user!.user_role !== 'super_admin') {
      ...
      for (const sysField of systemFields) delete fields[sysField];
    }
    ...
    const result = await transaction(async (client) => {
      ...
      if (rawKeys.length > 0) {
        ...
        updated = await client.query(
          `UPDATE assigned_works SET ${setClause} WHERE id = $${values.length} RETURNING *`, values
        );
      } else {
  ...
  try {
    ...
    const result = await transaction(async (client) => {
      const inserted = await client.query(
        'INSERT INTO work_milestones (work_id, title, target_date, status) VALUES ($1,$2,$3,$4) RETURNING *',
        [req.params.id, title, target_date, status || 'pending']
      );
      await recalculateProgressFromMilestones(client, req.params.id);
      return inserted;
    });
...
router.put('/:id/milestones/:milestoneId', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (req.body.milestone_description !== undefined) fields.title = req.body.milestone_description;
    if (req.body.target_date !== undefined) fields.target_date = req.body.target_date;
    if (req.body.expected_outcome !== undefined) fields.expected_outcome = req.body.expected_outcome;
    if (req.body.status !== undefined) fields.status = req.body.status;
    if (req.body.is_completed !== undefined) fields.status = req.body.is_completed ? 'completed' : 'pending';
  ...
  try {
    ...
    const problem = await transaction(async (client) => {
      const result = await client.query(
        ...
        [req.params.id, title || category || 'general', description, severity || impact_level || 'medium']
      );
