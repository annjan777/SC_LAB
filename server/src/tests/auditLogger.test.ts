...
export async function runAuditLoggerTest(): Promise<void> {
  ...
  const check = await query(
    `SELECT * FROM audit_logs WHERE action = 'TEST_AUDIT' AND entity_type = 'test_entity' ORDER BY performed_at DESC LIMIT 1`
  );
