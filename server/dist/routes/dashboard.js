...
router.get('/stats', authenticate, async (req, res) => {
    try {
        ...
        // Get active cycle
        const cycleResult = await query("SELECT id FROM work_cycles WHERE status = 'active' LIMIT 1");
        const activeCycleId = cycleResult.rows[0]?.id;
        if (isAdmin) {
            const [users, pendingPurchases, pendingLeaves, lowStock, teamWorks, repoDocs, facilities] = await Promise.all([
            ...
            let delayedCount = 0;
            if (activeCycleId) {
                const worksResult = await query('SELECT id FROM assigned_works WHERE cycle_id = $1', [activeCycleId]);
                for (const w of worksResult.rows) {
                    const pResult = await query(`SELECT status FROM progress_updates WHERE work_id = $1
             ORDER BY update_date DESC LIMIT 1`, [w.id]);
                    if (pResult.rows[0]?.status === 'delayed')
                        delayedCount++;
                }
            }
            // Recent docs
            ...
            const [myPurchases, myLeaves, inventory, myWorks, repoDocs, recentDocs] = await Promise.all([
                query("SELECT COUNT(*) FROM purchase_requests WHERE requested_by = $1 AND status IN ('submitted','in_progress')", [userId]),
                query("SELECT COUNT(*) FROM leave_requests WHERE requested_by = $1 AND status = 'pending'", [userId]),
                query('SELECT COUNT(*) FROM inventory_items'),
                activeCycleId
                    ? query('SELECT COUNT(*) FROM assigned_works WHERE user_id = $1 AND cycle_id = $2', [userId, activeCycleId])
                    : Promise.resolve({ rows: [{ count: '0' }] }),
                query(`SELECT COUNT(*) FROM repository_documents
           ...
           ORDER BY created_at DESC LIMIT 5`, [userId]),
            ...
            let avgCompletion = 0;
            if (activeCycleId) {
                const myWorkIds = await query('SELECT id FROM assigned_works WHERE user_id = $1 AND cycle_id = $2', [userId, activeCycleId]);
                if (myWorkIds.rows.length > 0) {
                    let total = 0;
                    for (const w of myWorkIds.rows) {
                        const p = await query('SELECT completion_percentage FROM progress_updates WHERE work_id = $1 ORDER BY update_date DESC LIMIT 1', [w.id]);
