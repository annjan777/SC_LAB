import { SC_LAB_RBAC_CONFIG, PERMISSION_POLICIES } from '../config/rbacConfig.js';
export function mapMethodToAction(method) {
...
export function expandUserRoles(userRole) {
    ...
    if (['LAB_MANAGER', 'RESEARCHER'].includes(normalizedRole))
        normalizedRole = 'COORDINATOR';
    const queue = [normalizedRole];
...
export function matchResourcePattern(pattern, path) {
